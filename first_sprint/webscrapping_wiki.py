...
links = ['https://www.91mobiles.com/phonefinder.php', "./data/mobiles/urls/urls.json", "./data/mobiles/mainpage/keys.json"]
...
def search(link, page_lnk=0):
    try:
        ...
        if page_lnk > 0:
            ...
            lnk = f"""arguments[0].setAttribute('onclick', 'return submitPage("next", "{page}");')"""
            driver.execute_script(lnk, nxt_btn[0])
            nxt_btn[0].click()
            print("retake from page: ", page)
            with open("./data/mobiles/urls/urls.json") as f:
                urls = json.load(f)
            with open("./data/mobiles/mainpage/keys.json") as f:
                cells_frmt = json.load(f)
            time.sleep(10)

        while len(driver.find_elements(by=By.CLASS_NAME, value="listing-btns4")) > 0:

            nxt_btn = driver.find_elements(by=By.CLASS_NAME, value="listing-btns4")
            print("len: ", len(nxt_btn))
            nxt_btn = nxt_btn[0].find_elements(by=By.CLASS_NAME, value="list-bttnn")
            ...
            nxt_btn[0].click()
            time.sleep(15)
            print("leaving page: ", page)
            page += 1

        with open(link[1], "w", encoding='utf-8') as f:
            json.dump(urls, f, indent=4, ensure_ascii=False)

        with open(link[2], "w", encoding='utf-8') as f:
        ...
        print(f"Error on page: {page}. saving point...")
        ...
        result = search(link, page_lnk=page)
